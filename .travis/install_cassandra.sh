...
if [[ "${CASS_VERSION}" == 1.2* ]]; then
    ...
    if [ "${CASS_VERSION}" == "1.2.19" ]; then
        wget http://www.apache.org/dyn/closer.cgi?path=/cassandra/1.2.19/apache-cassandra-1.2.19-bin.tar.gz && tar -xvzf apache-cassandra-1.2.19-bin.tar.gz && JVM_OPTS="-Xss512k" sudo sh apache-cassandra-1.2.19/bin/cassandra
    else
